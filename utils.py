...
def get_signin_status():
    data_login = {
        'uid': parameter.username,  # 此处填学号
        'upw': parameter.password,  # 此处填密码
        'smbtn': '进入健康状况上报平台',
...
def get_zhb_status():
    result = 'y'
    try:
        if parameter.zhb_parameter is None:
            return result
        url_zhb = "https://unified-area-code-n-service.jianguan.henan.gov.cn/nucleicapi/nucvac/info"
        data_zhb = '{"param":"'+parameter.zhb_parameter+'","_t":'+str(int(time()))+'}'
        header_zhb = {
...
def send_message(message: str):
    if parameter.send_type == 'bark':
        if parameter.send_parameter[-1] != '/':
            http.request(method='GET', url=parameter.send_parameter + '/健康打卡/' + message)
        else:
            http.request(method='GET', url=parameter.send_parameter + '健康打卡/' + message)
        logger.info("已使用"+parameter.send_type+"模式发送消息“"+message+"”")

    elif parameter.send_type == 'serverchan':
        http.request(method='GET', url='https://sctapi.ftqq.com/' + parameter.send_parameter + '.send?title=' + quote(message))
        logger.info("已使用"+parameter.send_type+"模式发送消息“"+message+"”")

    elif parameter.send_type == 'email':
        # send_parameter应为json字符串，格式为：{"host": "smtp服务器地址", "user": "邮箱登录名", "password": "邮箱密码", "receiver": "接收邮件的邮箱"}
        try:
            par = json.loads(parameter.send_parameter)
